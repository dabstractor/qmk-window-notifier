...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    // Add this near the start of the run function for additional debugging
    let mut debug_file = OpenOptions::new()
        .create(true)
        .write(true)
        .append(true)
        .open("/tmp/qmk-notifier-env.log")
        .unwrap_or_else(|_| panic!("Failed to open debug log file"));

    use std::io::Write;
    writeln!(debug_file, "---------- Environment Variables ----------").unwrap();
    for (key, value) in std::env::vars() {
        writeln!(debug_file, "{}: {}", key, value).unwrap();
    }
...
fn get_config_path() -> Result<PathBuf, Box<dyn Error>> {
    ...
    let debug_file_result = fs::OpenOptions::new()
        .create(true)
        .write(true)
        .append(true)
        .open("/tmp/qmk-notifier-debug.log");

    let mut debug_file = match debug_file_result {
        Ok(file) => Some(file),
        Err(e) => {
            eprintln!("Warning: Could not open debug log: {}", e);
            None
        }
    };

    // Helper closure for debug logging - make it mutable
    let mut log_debug = |message: &str| {
        if let Some(file) = &mut debug_file {
            use std::io::Write;
            let _ = writeln!(file, "{}", message);
        }
    };

    log_debug("Getting configuration path...");
    ...
    for path in config_paths {
        log_debug(&format!("Checking path: {}", path.display()));
        if path.exists() {
            log_debug(&format!("Found config at: {}", path.display()));
            return Ok(path);
        }
    }
